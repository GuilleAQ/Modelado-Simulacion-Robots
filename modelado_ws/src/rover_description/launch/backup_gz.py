...
def start_gzserver(context, *args, **kwargs):
    ...
    start_gazebo_server_cmd = IncludeLaunchDescription(
        ...
        launch_arguments={'gz_args': f'-r -s -v 4 {world}'}.items()
    )
...
def generate_launch_description():
    ...
    declare_sim_time = DeclareLaunchArgument(
        'use_sim_time', default_value='true', description='use_sim_time simulation parameter'
    )
    model_path = ''
    ...
    gz_image_bridge_node = Node(
        package='ros_gz_bridge',
        executable='parameter_bridge',
        arguments=['/front_camera/image', 'camera/image'],
        output='screen',
        parameters=[{
            'use_sim_time': True,
...
# Launch description
def generate_launch_description():
    moveit_config = MoveItConfigsBuilder("robot", package_name="rover_moveit_config").to_moveit_configs()
    declare_sim_time = DeclareLaunchArgument(
